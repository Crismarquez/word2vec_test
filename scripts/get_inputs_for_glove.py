...
print("lexical diversity: ", "{}%".format(round(len(set(corpus)) / len(corpus) * 100,3)))
...
co_occurrence = glove.co_occurrence.cooccurrences(corpus, vocabulary, S_WINDOW)
...
files = [vocabulary, co_occurrence, theta]

for file, name_file in zip(files, files_name):
    with open(path + name_file + '.pkl', 'wb') as f:
